...
import { ToolOutlined, CloseOutlined, InboxOutlined, CheckOutlined } from "@ant-design/icons";
...
export function ActionButtons({ updateStatus, availableActions }: ActionButtonsProps) {
    ...
    const availableButtons = availableActions.map((actionObject: any) => {
        ...
        const { color, icon, text, variant, successMessage, errorMessage, commentMessage } = actionButtonProps[action]

        const handleClick = (event: MouseEventHandler<HTMLButtonElement>, additionalParams = {}) => {
        ...
        return <Button icon={icon ?? null} color={color} variant={variant} onClick={handleClick}>{text}</Button>
    })
