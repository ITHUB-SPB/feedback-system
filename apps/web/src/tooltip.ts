...
export function initTooltips() {
  ...
  const tooltipButtons: NodeListOf<HTMLButtonElement> =
    document.querySelectorAll(".form-info-button");

  const tooltips = [...tooltipButtons].map((buttonElement) => {
    const tooltipElement: HTMLDivElement =
      buttonElement.parentElement!.querySelector(".form-info-tooltip")!;
    return {
      buttonElement,
      tooltipElement,
    };
  ...
  for (const { buttonElement, tooltipElement } of tooltips) {
    for (const [event, handler] of Object.entries(listeners)) {
      buttonElement.addEventListener(event, () =>
        handler(tooltipElement, buttonElement),
      );
    }
  }
}
