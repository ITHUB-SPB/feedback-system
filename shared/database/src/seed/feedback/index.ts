...
type FeedbackIn = PersonIn & FeedbackTypeIn & { project_id: number } & { description: string }
...
function generateProjectId(): { project_id: number } {
  const project_ids = {
    priozersk: [801, 957, 1090, 1099, 1105],
    vyborg: [1432, 1357, 1258, 1237, 1228],
    schlisselburg: [1016, 1034, 1333, 1350, 1417]
  }

  return { project_id: faker.helpers.arrayElement(Object.values(project_ids).flat()) }
}
...
async function generateFiles(): Promise<{ files: Blob[] }> {
  const images = ["image-1.png", "image-2.png", "image-3.png", "image-4.png", "image-5.jpg", "image-6.jpg"]
  ...
  return {
    files: await Promise.all(filePromises).then(buffers => buffers.map((buffer, index) => {
      return new File([buffer], fileNames[index]!, { type: mimeTypes[index] })
    }))
  }
}
